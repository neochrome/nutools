...
namespace NuTools.Specs
{
	...
	public class OptionParserSpecs
	{
		...
		{
			public void should_receive_one_value()
			{
				var receivedValues = new List<string>();
				var opts = new OptionParser();
				opts.Arg<string>("SINGLE", "A single value").Do(receivedValues.Add);

				opts.Parse(new[] { "one", "two" });

				Verify.That(() => receivedValues.SequenceEqual(new[] { "one" }));
			}
			...
			public void should_receive_array_of_consecutive_arguments()
			{
				var receivedValues = new string[0];
				var opts = new OptionParser();
				opts.Args<string>("MULTIPLE", "").Do(v => receivedValues = v);
				opts.Parse(new[] { "one", "two" });
				Verify.That(() => receivedValues.SequenceEqual(new[] { "one", "two" }));
			}

			public void should_handle_mix_of_single_and_multiple_valued_arguments()
			{
				var single = new List<string>();
				var multiple = new List<string>();
				var opts = new OptionParser();
				opts.Required.Arg<string>("SINGLE", "").Do(single.Add);
				opts.Args<string>("MULTIPLE", "").Do(multiple.AddRange);
				opts.Parse(new[] { "one", "two", "three" });
				Verify.That(() => single.SequenceEqual(new[] { "one" }));
				Verify.That(() => multiple.SequenceEqual(new[] { "two", "three" }));
			}
		}
